...
describe("통합 테스트", () => {
	...
	test("번들을 통한 요청/응답 흐름 테스트", async () => {
		...
		const swaggerConfig = (SwaggerUIBundle as unknown as jest.Mock).mock
			.calls[0][0];
		...
		const executeRequestWrapper =
			swaggerConfig.plugins[0].statePlugins.spec.wrapActions.executeRequest;
		const wrappedExecuteRequest = executeRequestWrapper(
			mockOriginalAction,
			mockSystem
		);
